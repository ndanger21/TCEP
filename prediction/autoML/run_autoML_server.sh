...
ssh ${user}@${host} 'docker run -d -v $PWD:/opt/nb --name tcep-automl-server -p 8888:8888 tcep-automl-server /bin/bash -c "mkdir -p /opt/nb && jupyter notebook --notebook-dir=/opt/nb --ip='0.0.0.0' --port=8888 --no-browser --allow-root"'
