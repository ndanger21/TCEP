...
nPublishers=$4
combinedPIM=$5
echo "time between events: $timeBetweenEvents"
...
res=`docker run --name containernet -d --rm --privileged \
    ...
    nohup python3 mininet_yahoo.py ${simDuration} ${algorithm} ${baseLatency} $USER $registry_user ${nPublishers} ${combinedPIM}`
