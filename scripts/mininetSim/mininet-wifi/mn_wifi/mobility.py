...
class Mobility(object):
                ...
                if dist >= 0.01:
                    if intf.bgscan_threshold or intf.active_scan \
                        ...
                        if wmediumd_mode.mode != w_cst.WRONG_MODE:
                            ...
                            #if self.count < 100:
                                #info('ap_in_range %s: intf %s, ap %s, node.position: %s node.pos: %s \n' % (dist, intf, ap, intf.node.position, intf.node.pos))
                            self.count += 1
    ...
    def do_handover(self, intf, ap_intf):
        ...
        changeAP = False
        handover_start = datetime.datetime.now()
        ...
        if self.check_if_ap_exists(intf, ap_intf):
            if not intf.associatedTo or changeAP:
                if ap_intf.node != intf.associatedTo:
                    prevAP = intf.associatedTo
                    assoc_infra_start = datetime.datetime.now()
                    ...
                    for ap in self.aps:
                        # OF1.3
                        # direct command format: dpctl tcp:127.0.0.1:6634 flow-mod cmd=del,table=0 eth_dst=aa:3c:5e:f2:33:e2
                        subprocess.Popen(['util/m', ap.name, 'dpctl', 'tcp:127.0.0.1:%s' % ap.listenPort, 'flow-mod', 'cmd=del,table=0', 'eth_src=%s' % intf.node.MAC(), '>&', '/dev/null'])
                        subprocess.Popen(['util/m', ap.name, 'dpctl', 'tcp:127.0.0.1:%s' % ap.listenPort, 'flow-mod', 'cmd=del,table=0', 'eth_dst=%s' % intf.node.MAC(), '>&', '/dev/null'])
                        # OF1.0
                        #subprocess.Popen(['util/m', ap.name, 'dpctl', 'del-flows', 'tcp:127.0.0.1:%s' % ap.listenPort,  'dl_src=%s' % intf.node.MAC(), '>&', '/dev/null'])
                        #subprocess.Popen(['util/m', ap.name, 'dpctl', 'del-flows', 'tcp:127.0.0.1:%s' % ap.listenPort,  'dl_dst=%s' % intf.node.MAC(), '>&', '/dev/null'])

                    #info('flushed all flows for %s on all APs \n' % intf.node)
                    #sleep(2.0)
                    # OF1.3
                    #subprocess.call(['util/m', intf.associatedTo.name, 'dpctl', 'tcp:127.0.0.1:%s' % intf.associatedTo.listenPort, 'stats-flow', 'table=0', 'eth_dst=%s' % intf.node.MAC()])
                    # OF1.0
                    #info('flows on %s: ' % intf.associatedTo)
                    #subprocess.call(['util/m', intf.associatedTo.name, 'dpctl', 'dump-flows', 'tcp:127.0.0.1:%s' % intf.associatedTo.listenPort, 'dl_dst=%s' % intf.node.MAC()])
    ...
    def associate_interference_mode(self, intf, ap_intf):
        if intf.bgscan_threshold or (intf.active_scan and 'wpa' in intf.encrypt):
            if not intf.associatedTo:
                #info('intf.bgscan_threshold: %s, intf.associatedTo: %s' % (intf.bgscan_threshold, intf.associatedTo))
                intf.associate_infra(ap_intf)
        ...
        for node in nodes:
            for wlan, intf in enumerate(node.wintfs.values()):
                if isinstance(intf, adhoc) or isinstance(intf, mesh) or \
                    ...
                    for ap in self.aps:
                        for ap_wlan, ap_intf in enumerate(ap.wintfs.values()):
                            #info('ap_intf: %s mesh %s adhoc %s wmdeium.mode %s' % (ap_intf, isinstance(ap_intf, mesh), isinstance(ap_intf, adhoc), wmediumd_mode.mode))
