...
class UserAP(AP, UserSwitch):
    ...
    def dpctl(self, *args):
        ...
        # for openflow1.3 from https://github.com/CPqD/ofsoftswitch13
        # dpctl tcp:127.0.0.1:6634 COMMAND
        # dpctl tcp:127.0.0.1:6634 flow-mod cmd=del,table=0 eth_dst=aa:3c:5e:f2:33:e2
        #return self.cmd('dpctl %s %s' % (listenAddr, args[0]))
        # customize this command since default mininet version does not work with arguments
        return self.cmd('dpctl', args[0], listenAddr, args[1])
