...
class IntfWireless(Intf):
    ...
    def handover_ieee80211r(self, ap_intf):
        ...
        subprocess.call(['util/m', self.node.name, 'wpa_cli', '-i %s' % self.name, 'roam %s' % ap_intf.mac])
    ...
    def associate_noEncrypt(self, ap_intf):
        ...
        info('using associate_noEncrypt for %s -> %s' % (self, ap_intf))
    ...
    def associate_infra(self, ap_intf):
        associated = 0
        #if self.name == 'car1-wlan0':
            #info('calling associate_infra(%s, %s), self.encrypt: %s, self.associatedTo: %s, ap_intf.ieee80211r: %s, ap_intf.encrypt: %s\n'
            # % (self, ap_intf, self.encrypt, self.associatedTo, ap_intf.ieee80211r, ap_intf.encrypt))
        ...
        if ap_intf.ieee80211r and (not self.encrypt or 'wpa' in self.encrypt):
            if not self.associatedTo:
                ...
                if wpa_file_exists:
                    info('not associated, wpa file exists, calling self.handover_ieee80211r(%s)' % ap_intf) if self.name == 'car1-wlan0' else None
                    self.handover_ieee80211r(ap_intf)
                else:  # initial call
                    info('not associated, no wpa file, calling self.wpa(%s)' % ap_intf) if self.name == 'car1-wlan0' else None
                    self.wpa(ap_intf)

            else:
                #info(' already associated, calling self.handover_ieee80211r(%s)' % ap_intf) if self.name == 'car1-wlan0' else None
                self.handover_ieee80211r(ap_intf)
            associated = 1
        elif not ap_intf.encrypt:
            associated = 1
            info(' no ap encrypt, calling self.associate_noEncrypt(%s)' % ap_intf) if self.name == 'car1-wlan0' else None
...
class master(TCWirelessLink):
    ...
    def __init__(self, node, wlan, port=None, intf=None):
        ...
        ########################
        # CHANGE TO MININET-WIFI
        #info('creating wireless interface %s in "master" mode' % self)
        # disable rxo and txo since it causes problems with UserAP
        #info('!!! disabling transmit and receive checksum offload (rxo, txo) on intf %s!!!\n' % self)
        #self.cmd('ethtool -K', self, 'tx off', 'rx off')
        ########################
...
class managed(TCWirelessLink):
    ...
    def __init__(self, node, wlan, intf=None):
        ...
        ########################
        # CHANGE TO MININET-WIFI
        #info('creating wireless interface %s in "managed"' % self)
        # disable rxo and txo since it causes problems with UserAP
        #info('!!! disabling transmit and receive checksum offload (rxo, txo) on intf %s!!!\n' % self)
        #self.cmd('ethtool -K', self, 'tx off', 'rx off')
        ########################
...
class wirelessLink(object):
    ...
    eqDelay = '(dist / 10) + 1'
    eqLatency = '(dist / 10)/2'
    # adjusted from 1.01 to 1.001 to have about 60% bandwidth at 500m distance (instead of 0.7%)
    eqBw = ' * (1.01 ** -dist)'
