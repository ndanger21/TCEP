...
[ -f ${work_dir}/docker-swarm.cfg ] || echo ".cfg file not found"
...
res=`docker run --name containernet -d --rm --privileged \
    ...
    nohup python3 mininet_wired_tree.py ${simDuration} ${algorithm} ${mobility} ${baseLatency} $HOME $registry_user ${nPublishers} ${nPublishersPerSection} ${combinedPIM}`
