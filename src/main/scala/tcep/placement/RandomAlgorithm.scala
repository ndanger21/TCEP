...
object RandomAlgorithm extends PlacementStrategy {
  ...
  /**
    ...
    */
  def applyRandomAlgorithm(operator: Query, candidateNodes: Vector[Member], dependencies: Dependencies)(implicit ec: ExecutionContext, cluster: Cluster): Future[HostInfo] = {
    val random: Int = scala.util.Random.nextInt(candidateNodes.size)
    val randomMember: Member = candidateNodes(random)
    for {
      _ <- this.initialize()
      bdpUpdate <- this.updateOperatorToParentBDP(operator, randomMember, dependencies.parents, Queries.estimateOutputBandwidths(operator)) } yield {
