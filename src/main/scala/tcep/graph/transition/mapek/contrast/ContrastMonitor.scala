...
class ContrastMonitor(mapek: MAPEK, consumer: ActorRef, fixedSimulationProperties: Map[Symbol, Int]) extends MonitorComponent(mapek) {
  ...
  def updateContextData(): Unit = {
    try {
      ...
      case e: Throwable => log.error(e, "failed to update context data")
    }
  }
  ...
  def calculateAverageHopsBetweenNodes: Double = {
    ...
    if(isLocalSwarm) 1
      ...
      if(networkHops.nonEmpty) networkHops.sum.toDouble / networkHops.size
      else 1
    }
  }
  ...
  def calculateAverageNodesInNHops(hops: Int): Double = {
    ...
    if(nodesInNHops.size > 0) nodesInNHops.values.sum.toDouble / nodesInNHops.size
    else 0
  }
...
object ContrastMonitor {
  ...
  def createContextDataUpdate(vivDistances: Map[(Member, Member), Option[Double]],
    ...
    try {
      ...
      case e: Throwable => log.error(e, "failed to create contextdata update map")
        throw e
    }
  }
